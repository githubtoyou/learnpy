...
def craw2(url):
    html = requests.post(url, data=json.dumps(body), headers=header)
    dumpJsonData = json.loads(html.text)
    return dumpJsonData["data"]["list"]

for user in craw2(url):
    article_title1 = user['article_title']
    print(article_title1)
